...
<template>
	<view class="uni-content">
		...
		<uni-popup ref="genderPopup" type="dialog">
			<uni-popup-dialog mode="base" title="选择性别" 
				@confirm="confirmGender">
				<view class="gender-options">
					<view v-for="item in genderOptions" :key="item.value" 
						class="gender-option" :class="{selected: selectedGender === item.value}"
  ...
  import {
    ...
    computed: {
		...
		methods: {
			...
			confirmAge(value) {
				  if (value === '') {
					mutations.updateUserInfo({ age: null });
				  ...
				  mutations.updateUserInfo({ age: age.toString() });
				  this.$refs.agePopup.close();
			},
