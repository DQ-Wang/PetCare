<template>
  <view class="pet-container">
    ...
    <uni-search-bar v-model="searchKeyword" placeholder="搜索" bgColor="#fff" :radius="60" cancelButton="none"
      style="margin: 30rpx;" @confirm="onSearch" @input="onSearch">
      <uni-icons slot="searchIcon" color="#bababa" size="40" type="home" />
    </uni-search-bar>

    <view class="category-section">
      <view class="segmented-control-wrap" v-if="!isSearching">
        <uni-segmented-control :current="current" :values="items" style-type="button" :active-color="'#b2d76e'"
        ...
        <view v-if="isSearching" class="category-body">
          ...
          <view v-else class="category-card" v-for="(item, index) in searchResults" :key="index" @click="enterService">
            ...
            <view class="title">
              ...
              <text class="price">¥{{ item.price }}<text v-if="item.unit"
                  style="font-size: 20rpx; font-weight: 300;">/{{ item.unit }}</text></text>
            </view>
          </view>
        </view>
